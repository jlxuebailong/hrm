//用户添加与修改页面
extjs.user.list = function (desktop) {
    //定义查询表单
    var search = {
    ...
    var ds = new Ext.data.Store({
        ...
        proxy: new Ext.data.HttpProxy({ url: '/Server/User.php?method=query', method: 'post' }),
    ...
    function save() {
        ...
        var win = desktop.getWindow('extjs.user.save.' + user.IntUserID);
        if (!win) {
            win = new extjs.user.save(desktop, user);
        }
        win.show();
    }
    //查看
    function detail() {
        ...
        var win = desktop.getWindow('extjs.user.detail.' + id);
        if (!win) {
            win = new extjs.user.detail(desktop, id);
        }
        win.show();
    }
    //移除
